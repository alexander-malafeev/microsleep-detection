...
for j in range(0,len(f_list)):
	...
	scores = Model.evaluate( generator_test, int(math.ceil((len(sample_list_test[j],)+0.0)/batch_size)), workers=1)
	generator_test = my_generator(data_test, targets_test, sample_list_test[j], shuffle = False)
	y_pred = Model.predict( generator_test, int(math.ceil((len(sample_list_test[j],)+0.0)/batch_size)), workers=1)
	print(y_pred.shape)
	...
	scores2 = Model.evaluate( generator_test, int(math.ceil((len(sample_list_test2[j],)+0.0)/batch_size)), workers=1)
	generator_test = my_generator(data_test, targets_test, sample_list_test2[j], shuffle = False)
	y_pred2 = Model.predict( generator_test, int(math.ceil((len(sample_list_test2[j],)+0.0)/batch_size)), workers=1)
